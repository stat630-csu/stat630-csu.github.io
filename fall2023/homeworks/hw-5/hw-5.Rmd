...
1. The data file [`weatherData.txt`](https://stat630-csu.github.io/homeworks/hw-5/weatherData.txt) has weather data for 11 different variables as given below.
    \begin{tabular}{l l}
...
for $x > 0$ and $\alpha, \beta > 0$.
    a) Draw $n = 20$ random observations from a gamma distribution with $\alpha = 5$ and $\beta = 1$ and store this data. Produce an image of the log-likelihood surface for $\alpha \in (2,8)$ and $\beta \in (.1, 2)$.  What do you notice?
    a) Write a method which numerically finds the maximum likelihood estimates $\alpha_{MLE}, \beta_{MLE}$ for this data, and obtain an approximate 95\% confidence interval for $\alpha$  based on the asymptotic properties of maximum likelihood.  Report the confidence interval.
    a) For the same data, write a method which will find profile likelihood based 95\% confidence intervals for $\alpha$.  Draw a plot which illustrates the profile likelihood-based CI, and report the confidence interval.
    a) Code a for-loop which will repeat the following simulation 1000 times:  (1) draw $n = 20$ random observations from this same gamma distribution, (2) find the 95\% Gaussian based confidence interval for $\alpha$, and (3) find the 95\% profile-likelihood based confidence interval for $\alpha$.  Report the coverage of the two different confidence interval methods.
    a) Of these two methods, which do you think gives the better confidence interval?  Explain.  (Can a plot help you explain your answer?)
  
3. In class, we fit a pairwise likelihood to data generated from the five dimensional model with cdf 
    $$
