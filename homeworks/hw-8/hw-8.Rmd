...
1. Consider the following model:
    ...
    a) Let $T_n = \sqrt{n} (\bar X_n - \mu)$. Show $\text{Var} T_n = \sum\limits_{k = -1}^{1}\left(1 - \frac{|k|}{n}\right) r(k)$ where $r(k) = \text{Cov}(X_1, X_{1 + k})$.
    
    b) Find the distribution of $T_n$ (note everything is normal here).
    
    c) Make a plot of the cdf of $T_n$.
    
    d) Write a function to draw a sample of size $n = 200$ from the MA(1) above.
    
    e) Repeat the following 5 times:
      ...
      f) Repeat c) for a NBB. 
      
      g) Which method do you prefer and why?
